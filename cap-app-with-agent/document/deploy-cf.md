...
The final step in SAP Business Application Studio is to deploy the application to the SAP BTP, Cloud Foundry runtime. This process also involves the automatic creation of destinations, enabling ODATA services to be utilized by other tools such as SAP Build Apps.

1. Install the dependencies for the Fiori application:

    ```bash
    cd app/incidents && npm install
    ```

2. Navigate back to the root folder and build the Multi-Target Application (MTA) archive:
