...
The prerequisites are the same as in the previous exercise.
...
1. Create a new custom business object
     ...
     > As the name of the business object must be unique and to distinguish the object for all participants in the workshop use the following naming convention: Add your initials plus a 3 digit number to the custom business object **CHECKLIST_\<YOUR_ID\>**, for example **GRCHECK_AB123**. 
...
2. Create the business object nodes and fields
   ...
   b) Select the **Nodes** tabulator and select CHECKLIST line and choose **New** button to add a new subnode CHECKLIST_ITEM.
      ![BO nodes](./img/bo_checklist_nodes.png)

   c) At the **Fields** tabulator select the CHECKLIST node. In addition to the autogenerated fields, choose **New** and add the following fields :
...
For our scenario we use the **Material Management - Purchase Order** catalog, as this is also required to access the Purchase Order API we use in a later exercise.
   ...
   1. Maintain catalogs
      ...
      a) Go back to the **Custom Business Objects** application and choose the CHECKLIST business object.
   ...
   2. Assign Business Catalog for **YY1_CHECKLIST_\<YOUR_ID\>_SCBO**
      ...
      a) Select **YY1_CHECKLIST_\<YOUR_ID\>_SCBO** at the **Custom Catalog Extension** table
...
### Task 3: Fill in the initial data
   ...
   1. Open generated UI
      ...
      b) In the **General Information** tabulator select **Go To Generated UI**
   ...
   3. Create item data
      ...
      b) Enter **Have the goods been ordered?** in the **CheckText** field and the select **Create** button.
